angular.module('healthMon').controller("dashboardCtrl", function($http,$state,authFact,moment) {
	...
	$http.get('common/getPatientList/'+authFact.getUserObj().HM_User_id).then(function(response) {
        vm.PatientList = response.data;
        vm.patientCount=0;        
		var plist=vm.PatientList;
        //to calculate count of alerts and tasks.
        for(var i in plist) {
			vm.patientCount++;
			plist[i].Created_Date=moment(plist[i].Created_Date).format('YYYY-MM-DD');
        }
    });
