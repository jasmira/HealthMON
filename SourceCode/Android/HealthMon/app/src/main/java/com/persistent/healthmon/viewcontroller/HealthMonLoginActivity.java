...
public class HealthMonLoginActivity extends Activity implements OnClickListener, LoginData {
    ...
    private String spUsername,spPassword;
...
//login without password
          /*  Intent intent = new Intent(getApplicationContext(), HealthMonMainActivity.class);
            ...
            if (mLoginCheckBox.isChecked()) {
                PreferanceManager.setBooleanPreferences("isLoginCheckBoxClicked",true,getApplicationContext());
                PreferanceManager.setLoginCredentials(getApplicationContext(), mUsername, mPassword);
            }
            arrayList= PreferanceManager.getLoginCredentials(HealthMonLoginActivity.this,mUsername);
            if(!arrayList.isEmpty()) {
                spUsername = arrayList.get(0);
                spPassword = arrayList.get(1);
            }
            if(mUsername==spUsername && mPassword==spPassword){
                Intent intent=new Intent(HealthMonLoginActivity.this,HealthMonMainActivity.class);
                startActivity(intent);
                finish();
            }
            else {
