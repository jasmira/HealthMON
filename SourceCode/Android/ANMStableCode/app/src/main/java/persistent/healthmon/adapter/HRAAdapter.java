...
public class HRAAdapter extends RecyclerView.Adapter<HRAAdapter.ViewHolder> {
    private static String TAG = HRAAdapter.class.getCanonicalName();
        ...
        holder.mRadioYes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(yes.isChecked()){
                    ...
                    ArrayList<String> lstsubOptns = mlstParseHRA.get(position).getSubQuestions();
                    final CharSequence[] items = lstsubOptns.toArray(new CharSequence[lstsubOptns.size()]);
                    if(items.length != 0) {
                        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
                        builder.setTitle("" + mlstParseHRA.get(position).getSurvey_Question_Description());
                        builder.setItems(items, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Toast.makeText(mContext, items[which] + " is selected", Toast.LENGTH_LONG).show();
                                mlstParseHRA.get(position).setAnswer(items[which].toString());
                            }
                        });
